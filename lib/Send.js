...
exports.info2xml = _.template([
  '<xml>',
    ...
    '<% }else if(type=="templateJobFinish"){  %>',
...
var Send = function (options) {
  ...
  if (!options.app) {
    if (this._options.url) {
      ...
      this._options.port = urlObj.port || this._options.port;
...
Send.prototype.templateJobFinish = function (status, msgId, cb) {
  this._request({
    status: status,
    msgId: msgId,
    type: 'templateJobFinish'
  }, cb);
};
